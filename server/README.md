...
The easiest way to get started is to run `make docker-compose`. That command will use docker compose to set up a new postgres database and start the server. The instant server will be available at http://instantdb.backend.medmatic.ai and you can connect to nrepl on port `6005`.
...
The instant server will run at [instantdb.backend.medmatic.ai](http://instantdb.backend.medmatic.ai) and you can connect to nrepl on port 6005.
